// Copyright (c) 2018-2019 The NRDI developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <znrdi/deterministicmint.h>
#include "znrditracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzNRDITracker::CzNRDITracker(std::string strWalletFile)
{
...
CzNRDITracker::~CzNRDITracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzNRDITracker::Init()
{
...
bool CzNRDITracker::Archive(CMintMeta& meta)
{
...
bool CzNRDITracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzNRDITracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzNRDITracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzNRDITracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzNRDITracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzNRDITracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzNRDITracker::GetSerialHashes()
{
...
CAmount CzNRDITracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzNRDITracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzNRDITracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzNRDITracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzNRDITracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzNRDITracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzNRDITracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzNRDITracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzNRDITracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzNRDITracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzNRDITracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzNRDIWallet* zNRDIWallet)
{
    bool iszNRDIWalletInitialized = (NULL != zNRDIWallet);
    CMintMeta meta;
    ...
    if (! iszNRDIWalletInitialized)
        zNRDIWallet = new CzNRDIWallet(strWalletFile);
    meta.isSeedCorrect = zNRDIWallet->CheckSeed(dMint);
    if (! iszNRDIWalletInitialized)
        delete zNRDIWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzNRDITracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNRDITracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzNRDITracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzNRDITracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzNRDITracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzNRDITracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzNRDIWallet* zNRDIWallet = new CzNRDIWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zNRDIWallet);
        }
        delete zNRDIWallet;
        LogPrint("zero", "%s: added %d dznrdi from DB\n", __func__, listDeterministicDB.size());
    }
