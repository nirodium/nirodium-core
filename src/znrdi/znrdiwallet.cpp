...
CzNRDIWallet::CzNRDIWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing znrdi seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZNRDISeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZNRDISeed_deprecated()) {
                    LogPrintf("%s: Updated zNRDI seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old znrdi seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of znrdi wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzNRDIWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZNRDICount(nCountLastUsed);
    else if (!walletdb.ReadZNRDICount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzNRDIWallet::Lock()
{
    seedMaster = 0;
}

void CzNRDIWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNRDIWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZNRDI(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzNRDIWallet::LoadMintPoolFromDB()
{
...
void CzNRDIWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNRDIWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNRDIWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->znrdiTracker->HasPubcoinHash(pMint.first)) {
...
bool CzNRDIWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZNRDI(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to znrdiTracker which also adds to database
    pwalletMain->znrdiTracker->Add(dMint, true);
...
void CzNRDIWallet::SeedToZNRDI(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzNRDIWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNRDIWallet::GenerateDeterministicZNRDI(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzNRDIWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNRDIWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzNRDIWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
