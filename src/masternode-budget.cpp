...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf, bool fBudgetFinalization)
{
    ...
    for (const CTxOut &o : txCollateral.vout) {
        ...
        if (fBudgetFinalization) {
            ...
            // Note: there are still old valid budgets out there, but the check for the new 5 NRDI finalization collateral
            //       will also cover the old 50 NRDI finalization collateral.
...
CAmount CBudgetManager::GetTotalBudget(int nHeight)
{
    ...
    CAmount nSubsidy = 0;
    if (nHeight <= 259200 && nHeight >= 151200) {  //default last_POW_Block = 259200
        nSubsidy = 50 * COIN;
