...
public:
    ...
    bool WriteZNRDISeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZNRDISeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZNRDISeed_deprecated(uint256& seed);
    bool EraseZNRDISeed();
    bool EraseZNRDISeed_deprecated();

    bool WriteZNRDICount(const uint32_t& nCount);
    bool ReadZNRDICount(uint32_t& nCount);
