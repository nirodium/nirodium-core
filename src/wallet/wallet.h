...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that znrdispends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT5000IFMN = 3,
    ONLY_NONDENOMINATED_NOT5000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 NRDI at the same time
    ONLY_5000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zNRDI send
enum ZerocoinSpendStatus {
    ZNRDI_SPEND_OKAY = 0,                            // No error
    ZNRDI_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZNRDI_WALLET_LOCKED = 2,                         // Wallet was locked
    ZNRDI_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZNRDI_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZNRDI_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZNRDI_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZNRDI_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZNRDI_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZNRDI_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZNRDI_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZNRDI_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZNRDI_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZNRDI_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZNRDI_SPENT_USED_ZNRDI = 14,                      // Coin has already been spend
    ZNRDI_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZNRDI_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZNRDIOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fznrdiAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzNRDITracker> znrdiTracker;
    ...
    void setZWallet(CzNRDIWallet* zwallet)
    {
        zwalletMain = zwallet;
        znrdiTracker = std::unique_ptr<CzNRDITracker>(new CzNRDITracker(strWalletFile));
    }

    CzNRDIWallet* getZWallet() { return zwalletMain; }
    ...
    /** zNRDI reset */
    boost::signals2::signal<void()> NotifyzNRDIReset;
