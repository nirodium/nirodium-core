...
#include "ui_znrdicontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->znrdiTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->znrdiTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zNRDI.";
            else if (!mint.isSeedCorrect)
                strReason = "The zNRDI seed used to mint this zNRDI is not the same as currently hold in the wallet";
            else
