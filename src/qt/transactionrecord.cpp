...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZNRDIModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->znrdiTracker->HasMintTx(hash))) {
            //zNRDI stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZNRDI;
...
bool IsZNRDIType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZNRDI:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZNRDI || type == TransactionRecord::MNReward) {
