...
// The zNRDI block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zNRDI is a hash of the serial
...
{
    //Create an output returning the zNRDI that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZNRDIOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zNRDI output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zNRDI", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZNRDIOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zNRDI output", __func__);
...
{
    CzNRDITracker* znrdiTracker = pwallet->znrdiTracker.get();
    CMintMeta meta;
    if (!znrdiTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    znrdiTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a NRDI stake is the outpoint
